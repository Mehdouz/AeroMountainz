      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
          <div className="lg:sticky lg:top-32 self-start">
            ...
            <StyledHeading
              heading={heading}
              className="font-serif text-4xl lg:text-6xl font-light text-[var(--text-primary)] leading-tight mb-6"
            />

            {footer && (
              <div className="mt-8 inline-flex items-center gap-3 border border-[var(--border-gold)] rounded-full px-5 py-3">
                <div className="w-1.5 h-1.5 rounded-full bg-[var(--gold)]" />
                <span className="text-sm font-sans text-[var(--text-secondary)]">{footer}</span>
              </div>
            )}
          </div>
