          ...
          <Reveal threshold={0.1}>
            <div>
              ...
              <SplitHeading
                ...
                breakLines={false}
                className="font-serif font-light text-4xl sm:text-5xl lg:text-6xl text-text-primary leading-[1.1]"
              />
              {data.subtitle && (
                <p className="mt-7 mb-8 font-serif italic text-xl lg:text-2xl text-(--champagne)/85 font-light">
                  {data.subtitle}
                </p>
              )}
              {data.body && (
                <div className="space-y-4 text-base lg:text-[15px] text-(--ink-65) leading-[1.75] font-sans [&_p]:mb-0">
              ...
              {data.linkLabel && data.linkHref && (
                <Link
                  ...
                  className="inline-flex items-center gap-2 mt-6 pb-1 font-serif italic text-lg text-champagne border-b border-champagne-line hover:border-champagne transition-colors group"
                >
                  {data.linkLabel}
          ...
          <Reveal threshold={0.1} delay={150}>
            <div className="bc-experience-gallery grid grid-cols-2 gap-3 lg:gap-3.5 auto-rows-[180px] sm:auto-rows-[200px] lg:auto-rows-[200px]">
              {data.gallery.slice(0, 4).map((item, i) => (
                <div
                  key={i}
                  className={`bc-experience-gallery-item relative overflow-hidden rounded-xl bg-midnight group ${
                    i === 0 ? 'row-span-2' : ''
                  }`}
                >
                  <Image
