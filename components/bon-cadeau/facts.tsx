  ...
  return (
    <section className="section-midnight py-24 lg:py-32">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 border-t border-l border-(--champagne)/15">
          {data.items.map((item, i) => (
            <Reveal
              key={i}
              threshold={0.1}
              delay={(i % 3) * 80}
              className="border-r border-b border-(--champagne)/15"
            >
              <div className="px-8 py-9 lg:px-9 lg:py-10 h-full transition-colors hover:bg-(--champagne)/[0.04]">
                <BonCadeauIcon
                  iconKey={item.iconKey}
                  className="w-7 h-7 text-champagne mb-5"
                />
                <h3 className="font-serif text-xl text-bone mb-2 font-normal">
                  {item.title}
                </h3>
                <p className="text-[13px] text-(--bone)/65 leading-[1.55] font-sans">
                  {item.description}
                </p>
              </div>
            </Reveal>
