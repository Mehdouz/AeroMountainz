...
function Stars({ count }: { count: number }) {
  ...
  return (
    <span
      className="text-[var(--champagne)] tracking-[4px] font-sans"
        ...
        <Reveal threshold={0.1}>
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6 mb-14 lg:mb-16">
            ...
            {(data.googleRatingStars || data.googleRatingLabel) && (
              <div className="inline-flex items-center gap-4 px-5 py-3 border border-[var(--champagne)]/25 rounded">
                {data.googleRatingStars && (
                  <span className="font-serif text-2xl text-[var(--champagne)]">
                    {data.googleRatingStars} ★
                  </span>
                )}
                {data.googleRatingLabel && (
                  <span className="text-[11px] tracking-[0.15em] uppercase text-[var(--bone)] font-sans leading-tight whitespace-pre-line">
                    {data.googleRatingLabel}
                  </span>
                )}
              </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {data.items.map((item, i) => (
            <Reveal key={i} threshold={0.1} delay={i * 120}>
              <article className="h-full p-8 rounded border border-[var(--champagne)]/15 bg-white/[0.025] hover:bg-white/[0.04] hover:border-[var(--champagne)]/30 transition-colors">
                <Stars count={item.stars} />
                <p className="mt-5 mb-6 text-[15px] text-[var(--bone)]/90 leading-[1.65] font-sans min-h-[100px]">
                  {item.quote}
                </p>
                <div className="pt-4 border-t border-[var(--champagne)]/15 flex justify-between items-baseline gap-3">
                  <span className="text-sm text-[var(--bone)] font-medium font-sans tracking-wider">
                    {item.name}
                  </span>
                  {item.occasion && (
                    <span className="font-serif italic text-sm text-[var(--champagne)]/75">
                      {item.occasion}
                    </span>
                  )}
                </div>
